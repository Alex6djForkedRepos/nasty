...
in {
  ...
  config = lib.mkMerge [
    ...
    #
    # Auth: requires HTTP basic auth via an htpasswd file the engine
    # generates and persists. Previously ran --no-auth, which meant
    # anyone reaching port 8000 could push backups OR delete existing
    # repos. The htpasswd file is generated by nasty_system::rest_server
    # before the protocol-enable hook starts this service (see
    # prepare_protocol in protocol.rs), and is also re-generated by
    # the rotate_credentials RPC. If the file is missing at start
    # time, the service fails to start LOUDLY rather than silently
    # falling back to --no-auth — preventing an unauthenticated
    # write surface from existing for any window.
    systemd.services.nasty-rest-server = {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "nasty-rest-server-start" ''
          ...
          # htpasswd file: generated by the engine before this service
          # starts (see nasty_system::rest_server::ensure_credentials,
          # called from prepare_protocol). Fail loud if it's missing
          # rather than silently serving an unauthenticated write
          # surface — an upgrade from a pre-auth NASty version should
          # restart nasty-engine to trigger credential generation.
          HTPASSWD="/var/lib/nasty/rest-server.htpasswd"
          if [ ! -f "$HTPASSWD" ]; then
            echo "nasty-rest-server: ERROR htpasswd file $HTPASSWD missing. The engine generates this on protocol enable. If you see this on an upgrade from a pre-auth NASty version, restart nasty-engine to trigger credential generation, or call the engine RPC service.rest_server.credentials via the WebUI." >&2
            exit 1
          fi
          ...
          exec ${pkgs.restic-rest-server}/bin/rest-server \
            --listen :8000 --path "$REPO_PATH" --htpasswd-file "$HTPASSWD" $TLS_ARGS
        '';
    ...
    services.caddy = mkIf (cfg.webui.package != null) {
      ...
      package = pkgs.caddy.withPlugins {
        ...
        hash = "sha256-4iF976LfeFS3xmRmLUG9lfPnYkWc2tY5XyGaB32FYoM=";
      };
      globalConfig = ''
